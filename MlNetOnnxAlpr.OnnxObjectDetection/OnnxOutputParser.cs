...
namespace MLNetOnnxAlpr.OnnxObjectDetection
{
    public class OnnxOutputParser
    {
        ...
        public float[] ExtractClassProbabilities(float[] modelOutput, int row, int column, int channel, float confidence)
        {
            ...
            var classProbabilities = new float[classLabels.Length];
            for (var classProbability = 0; classProbability < classLabels.Length; classProbability++)
            {
        ...
        public List<BoundingBox> FilterBoundingBoxes(List<BoundingBox> boxes, int limit, float iouThreshold)
        {
            ...
            for (var i = 0; i < boxes.Count; i++)
            {
                ...
                for (var j = i + 1; j < boxes.Count; j++)
                {
                    ...
                    if (IntersectionOverUnion(sortedBoxes[i].Rectangle, sortedBoxes[j].Rectangle) > iouThreshold)
                    {
